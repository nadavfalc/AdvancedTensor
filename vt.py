...
counter = 0
def decimal_to_binary_array(n, pad):
    binary = []
    while n > 0:
        binary.append(n % 2)
        n //= 2
    binary.reverse()
    binary = np.append(np.zeros(pad - len(binary)), binary)
    return binary


class VTCode:
    def __init__(self, n: int, q: int, a = 0, b = 0,
                correct_substitutions = False, systematic_locs=None):
        '''
    ...
    def fali_vt_encoder2(self, x: np.array):
        """
        encode a codeword of length k to a VT_0(self.n) codeword with systematic positions -2^j j=0,1,...,|log_2(n+1)|-1
        """
        global counter
        counter+=1
        ...
        systematic_locs = [(-(2 ** j)-1) % (self.n + 1) for j in range(self.n-self.k)]
        c_i = 0
        
        for i in range(self.n):
            if i not in systematic_locs:
                c_[i] = x[c_i]
                c_i += 1        
        # i+1 and not mod i because in cs we start from 0
        syn_c_ = sum((i+1) * c_i for i, c_i in enumerate(c_)) % (self.n+1)
        bin_a = decimal_to_binary_array(syn_c_, self.n)[::-1]
        if bin_a is not None and bin_a != []:
            for i, b in enumerate(bin_a):
                c_[systematic_locs[i]] = b
        assert self._is_codeword(c_)
        return np.array(c_)


    def fali_vt_encoder(self, x: np.array):
        """
        ...
        for i in range(self.n):
            if i not in self.systematic_locs:
                c_[i] = x[c_i]
                c_i += 1        
                
        # i+1 and not mod i because in cs we start from 0
        syn_c_ = sum((i+1) * c_i for i, c_i in enumerate(c_)) % (self.n + 1)
        for i in range(2 ** len(self.systematic_locs)):
            bin_a = decimal_to_binary_array(i, pad=len(self.systematic_locs))
            syn = (np.sum(np.multiply(self.systematic_locs+np.ones(len(self.systematic_locs)), bin_a))) % (self.n + 1)
            if syn == (- syn_c_) % (self.n + 1):
                break
        
        if bin_a is not None and bin_a != []:
            for i, b in enumerate(bin_a):
                c_[self.systematic_locs[i]] = b
        assert self._is_codeword(c_), (self.n, syn_c_, bin_a, c_)
        return np.array(c_)
    
    
    def fali_vt_decoder(self, c: np.array):
        y = _correct_binary_indel(self.n, self.m, self.a, c)
        if y == []:
            exit("error in decoding!")
        
        x_ = np.empty(0)
        for i in range(self.n):
            if i not in self.systematic_locs:
                x_ = np.append(x_, [y[i]])
        return y, np.array(x_)
        
    def fali_vt_decoder2(self, c: np.array):
        y = _correct_binary_indel(self.n, self.m, self.a, c)
        if y == []:
            exit("error in decoding!")
        x_ = np.empty(0)
...
def number2digit(num, k):
    arr = [int(d) for d in bin(num)[2:]]
    arr = [0] * (k - len(arr)) + arr
    return np.flip(np.array(arr))
    
    
def create_string(numbers):
    """
    Receives a list of one-digit numbers and returns a string created from them.
    """
    # Use a list comprehension to convert each number to a string
    # and join them into a single string using the str.join() method
